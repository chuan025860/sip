...
public class CustomerLoginController {
    ...
    @ResponseBody
    public Map<String, String> checkCustomerLogin(@RequestParam("email") String email,
        ...
        if (customer != null) {
            ...
            response.put("message", "用戶帳號或密碼錯誤");
        }
        return response;
    }
    ...
    public String indexLogout(HttpSession httpSession, HttpServletResponse response) {
        if (httpSession != null) {
            httpSession.invalidate();
            // 刪除 JWT Cookie
            Cookie jwtCookie = new Cookie("Authorization", null);
