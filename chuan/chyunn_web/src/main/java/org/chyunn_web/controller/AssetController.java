...
import org.chyunn_web.service.AssetHistoryService;
import org.chyunn_web.service.AssetService;
import org.chyunn_web.service.InventoryEquipmentFileService;
import org.chyunn_web.service.LocationService;
...
public class AssetController {
    ...
    public ResponseEntity<Map<String, Object>> select_incident(
        ...
        try {
            ...
            for (Inventory_Equipment inventoryEquipment : inventoryEquipmentPage.getContent()) {
                Inventory_EquipmentDto dto = assetService.convertInventory_EquipmentDto(inventoryEquipment);
    ...
    public ResponseEntity<Map<String, Object>> getPropertyDetails(@RequestParam String id) {
        ...
        try {
            ...
            Inventory_EquipmentDto inventoryEquipmentDto = assetService.convertInventory_EquipmentDto(inventoryEquipment);
            List<AssetHistoryDto> assetHistoryDtos = new ArrayList<>();
            for (AssetHistory assetHistory : inventoryEquipment.getAssetHistoryList()) {
    ...
    public ResponseEntity<Map<String, Object>> updateProperty(
        ...
        Map<String, Object> response = new HashMap<>();
        try {
            Inventory_Equipment inventoryEquipment = assetService.getPropertyDetails(final_property_id);
            ...
            assetService.updateProperty(inventoryEquipment);
    ...
    public ResponseEntity<Map<String, Object>> addProperty(@RequestBody Inventory_Equipment inventoryEquipment) {
        ...
        try {
            Inventory_Equipment new_inventoryEquipment = assetService.saveProperty(inventoryEquipment);
            response.put("code", 200);
    ...
    public ResponseEntity<Map<String, Object>> get_search(
        ...
        Map<String, Object> response = new HashMap<>();
        try {
            Page<Inventory_Equipment> assetPage = assetService.getProperty(final_property_id, PageRequest.of(page, size));
            List<Inventory_EquipmentDto> inventoryEquipmentDtos = new ArrayList<>();
            for (Inventory_Equipment inventoryEquipment : assetPage.getContent()) {
                Inventory_EquipmentDto dto = assetService.convertInventory_EquipmentDto(inventoryEquipment);
                inventoryEquipmentDtos.add(dto);
            }
            response.put("code", 200);
            response.put("data", inventoryEquipmentDtos); // 返回查詢結果
            response.put("totalPages", assetPage.getTotalPages()); // 返回總頁數
            response.put("totalElements", assetPage.getTotalElements()); // 返回總資料數
    ...
    public ResponseEntity<Map<String, Object>> filterLocation(
        ...
        try {
            ...
            Page<Inventory_Equipment> assetPage = assetService.filterLocation(location, PageRequest.of(page, size));
    ...
    public void exportExcel(HttpServletResponse response) throws IOException {
        try {
            List<Inventory_Equipment> data = assetService.getAll(); // 可以替換成篩選條件
            response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
            String fileName = URLEncoder.encode("財產資料.xlsx", StandardCharsets.UTF_8);
            response.setHeader("Content-Disposition", "attachment; filename*=UTF-8''" + fileName);

            Workbook workbook = new XSSFWorkbook();
            Sheet sheet = workbook.createSheet("財產清單");
    ...
    public ResponseEntity<Map<String, Object>> generateRandomInventory(@RequestBody Map<String, Integer> request) {
        ...
        try {
            // 先檢查是否還有正在進行的盤點
            List<Inventory_Equipment> inProgress = assetService.findByInventoryInProgressTrue();
            ...
            List<Inventory_Equipment> candidates = assetService.findByInventoryCheckedFalseAndInventoryInProgressFalse();
            ...
            List<Inventory_Equipment> selected = candidates.stream().limit((int) count).toList();
            for (Inventory_Equipment e : selected) {
                e.setInventory_in_progress(true);                // 盤點中
                e.setState(false);                              // 尚未完成
            }
            assetService.saveInventoryData(selected);
            response.put("code", 200);
    ...
    public String excel_inventory_equipment_inster_datebase() {
        ...
        try (FileInputStream fis = new FileInputStream(new File(excelFilePath));
            ...
            assetService.insertInventoryList(inventoryequipmentList);
