...
public class MobileController {
    ...
    public ResponseEntity<Map<String, Object>> inventory_update(@RequestPart(value = "files", required = false) List<MultipartFile> files,
        ...
        try {
            Inventory_Equipment newInventoryEquipment = assetService.getPropertyDetails(inventoryEquipment.getFinal_property_id());
            // 查詢該設備目前的檔案數量
...
//                newInventoryEquipment.setIs_updated(true);
                ...
                try {
                    Inventory_Equipment updateSucuess = assetService.insertInventoryAndFiles(newInventoryEquipment, newInventoryEquipment.getInventoryEquipmentFiles());
                    Inventory_EquipmentDto inventoryEquipmentDto = assetService.convertInventory_EquipmentDto(updateSucuess);
                    List<InventoryEquipmentFileDto> inventoryEquipmentFileDtos = new ArrayList<>();
