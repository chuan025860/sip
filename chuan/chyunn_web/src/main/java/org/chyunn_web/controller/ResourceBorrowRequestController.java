...
public class ResourceBorrowRequestController {
    ...
    public ResponseEntity<Map<String, Object>> events(HttpServletRequest request) {
        ...
        Map<String, Object> response = new HashMap<>();
        List<ResourceBorrowRequest> resourceBorrowRequestList = resourceBorrowRequestServie.findAllVisibleToUser(userService.getUser(loginId));
        ...
        for (ResourceBorrowRequest resourceBorrowRequest : resourceBorrowRequestList) {
            ...
            String content = resourceBorrowRequest.getContent();
            String category = String.valueOf(resourceBorrowRequest.getCategory());
            String title;
            if ("PRIVATE_EVENT".equals(category)) {
                title = content;
            } else {
                title = item != null ? content + " - " + item : content;
            }
            ...
            // 根據 item 設定顏色
            String customColor = resourceBorrowRequest.getColor();
            if (customColor != null && !customColor.trim().isEmpty()) {
                // 若有自訂顏色，就使用它
                eventDTO.setBackgroundColor(customColor);
                eventDTO.setBorderColor(customColor);
            } else {
                // 2. 根據 item 名稱判斷預設顏色
                switch (item) {
    ...
    @PostMapping("/resource/personal-event")
    public ResponseEntity<Map<String, Object>> createPersonal_event(@RequestBody ResourceBorrowDTO dto) {
        User creator = userService.getUser(dto.getCreator());
        System.out.println(creator.getUsername());
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm");
        Map<String, Object> response = new HashMap<>();
        ...
        if (dto.isPublic_event()) {
            request.setCategory(ResourceBorrowRequest.Category.valueOf("PUBLIC_EVENT"));
        } else {
            request.setCategory(ResourceBorrowRequest.Category.valueOf("PRIVATE_EVENT"));
        }
        request.setNote(dto.getNote());
        request.setCreator(creator);
        request.setCreatedAt(LocalDateTime.now());
    ...
    public ResponseEntity<String> deleteResource(@RequestParam Integer id, HttpServletRequest request) {
        try {
            // 1. 查出該筆資源
            ResourceBorrowRequest resourceBorrowRequest = resourceBorrowRequestServie.getResourceBorrowRequestById(id);
            if (request == null) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body("找不到指定資源");
            }
            String token = "";
            Cookie[] cookies = request.getCookies();
            if (cookies != null) {
            ...
            // 3. 取得目前登入者的帳號與角色
            String loginId = jwtTokenProvider.getLoginIdFromToken(token); // 登入者 ID
            List<String> roles = jwtTokenProvider.getUserRolesFromToken(token); // 登入者角色清單

            // 4. 判斷是否是擁有者或具有 ROLE_ADMIN 權限
            boolean isOwner = loginId.equals(resourceBorrowRequest.getCreator().getLoginId());
            boolean isAdmin = roles.contains("ROLE_ADMIN");

            if (!isOwner && !isAdmin) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN).body("沒有刪除權限");
            }
            boolean deleted = resourceBorrowRequestServie.deleteById(id);
            if (deleted) {
            ...
            e.printStackTrace(); // 可換成 log.error(...)
