    ...
    public ResponseEntity<Map<String, Object>> easyRegister(@RequestParam(name = "USERID", required = false) String userId,
        ...
        if (userId != null && loginId != null && userName != null) {
            ...
            if (!userService.existsByLoginId(loginId)) {
                ...
                user.setEmployee_no(loginId);
                String encodedPwd = pwdEncoder.encode(loginId);
                ...
                try {
                    User saved = userService.saveReturnUser(user);
                    // 建立角色
                    List<UserRole> roles = new ArrayList<>();
                    UserRole role1 = new UserRole();
                    role1.setId(new UserRoleId(saved.getId(), "ROLE_USER"));
                    role1.setUser(saved);
                    roles.add(role1);
                    saved.setRoles(roles); // 設定進 user 的角色關係
                    userService.saveReturnUser(saved);
                ...
                if (dbUser.getUserId() == null) {
                    ...
                    userService.saveReturnUser(dbUser);
                }
